...
def create_cli() -> CLIBase:
    """Create a configured TextMatcher CLI instance."""
    return CLIBase(
        prog="textmatcher",
        description="Fuzzy text comparison, name matching, and duplicate detection",
        version="0.1.0",
    )
...
def _add_matcher_args(subparser: argparse.ArgumentParser) -> None:
    ...
    subparser.add_argument(
        ...
        type=float,
        dest="jaro_winkler",
        help="Jaro-Winkler threshold override (0.0-1.0)",
    )
    subparser.add_argument("--metaphone", type=_str_to_bool, help="Require metaphone match (true/false)")
    subparser.add_argument(
        "--debug",
        action="store_true",
        help="Enable debug mode with detailed explanations",
    )
...
def run_compare_names(args: argparse.Namespace, cli: CLIBase) -> None:
    ...
    fmt = getattr(args, "output_format", "summary")
    if fmt == "json":
        write_output({"match": is_match, "metrics": metrics}, args)
    else:
        status = "MATCH" if is_match else "NO MATCH"
...
def run_diff(args: argparse.Namespace, cli: CLIBase) -> None:
    ...
    if getattr(args, "file1", None):
        with open(os.path.realpath(args.file1), "r", encoding="utf-8") as f:
            text1 = f.read()
    if getattr(args, "file2", None):
        with open(os.path.realpath(args.file2), "r", encoding="utf-8") as f:
            text2 = f.read()
...
def run_similarity(args: argparse.Namespace, cli: CLIBase) -> None:
    ...
    fmt = getattr(args, "output_format", "summary")
    if fmt == "json":
        write_output({"text1": args.text1, "text2": args.text2, "similarity": score}, args)
    else:
...
def run_same_chars(args: argparse.Namespace, cli: CLIBase) -> None:
    ...
    if fmt == "json":
        ...
        write_output({"same_chars": result, "similarity": similarity}, args)
    elif result:
...
def run_algorithm_info(args: argparse.Namespace, cli: CLIBase) -> None:
    ...
    use_cases = AlgorithmSelector.get_all_use_cases()
    rows = [[uc.value, cfg["primary"], f"{cfg['speed_gain']:.1f}x"] for uc, cfg in use_cases]
...
def run_batch(args: argparse.Namespace, cli: CLIBase) -> None:
    ...
    with open(os.path.realpath(args.input), "r", encoding=getattr(args, "encoding", "utf-8")) as f:
...
def _parse_candidates(args: argparse.Namespace) -> List[str]:
    ...
    if cfile:
        with open(os.path.realpath(cfile), "r", encoding="utf-8") as f:
...
def _read_items(args: argparse.Namespace) -> List[str]:
    ...
    if input_file:
        with open(os.path.realpath(input_file), "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip()]
    return []
...
def _setup_subcommands(cli: CLIBase) -> None:
    ...
    p_cmp = cli.add_subcommand(
        "compare",
        "Compare two single words or names",
        handler=run_compare,
        aliases=["cmp"],
    )
    ...
    p_cn = cli.add_subcommand(
        "compare-names",
        "Compare multi-word full names",
        handler=run_compare_names,
        aliases=["cn"],
    )
    ...
    p_cn.add_argument(
        "--fuzzy-align",
        action="store_true",
        help="Use fuzzy alignment (e.g. ROBERT ≈ ROBERTO)",
    )
    _add_matcher_args(p_cn)

    # --- compare-text (phrases) ---
    p_ct = cli.add_subcommand(
        "compare-text",
        "Compare two phrases or sentences",
        handler=run_compare_text,
        aliases=["ct"],
    )
    ...
    # --- find-match ---
    p_fm = cli.add_subcommand(
        "find-match",
        "Find best match from candidates",
        handler=run_find_match,
        aliases=["fm"],
    )
    ...
    p_fm.add_argument(
        ...
        type=float,
        default=0.85,
        help="Minimum threshold (default: 0.85)",
    )
    _add_matcher_args(p_fm)

    # --- find-matches (top-K) ---
    p_fms = cli.add_subcommand(
        "find-matches",
        "Find top-K matches from candidates",
        handler=run_find_matches,
        aliases=["fms"],
    )
    ...
    # --- duplicates ---
    p_dup = cli.add_subcommand(
        "duplicates",
        "Detect duplicates in a list",
        handler=run_duplicates,
        aliases=["dup"],
    )
    ...
    p_dc.add_argument(
        "--ignore-comments",
        action="store_true",
        help="Remove comments before comparison",
    )
    _add_matcher_args(p_dc)

    # --- normalize ---
    p_norm = cli.add_subcommand(
        "normalize",
        "Normalize text for comparison",
        handler=run_normalize,
        aliases=["norm"],
    )
    ...
    p_norm.add_argument(
        "--case-mode",
        choices=["upper", "lower", "title", "none"],
        default=None,
        help="Case conversion mode",
    )
    _add_matcher_args(p_norm)

    # --- similarity ---
    p_sim = cli.add_subcommand(
        "similarity",
        "Simple similarity percentage",
        handler=run_similarity,
        aliases=["sim"],
    )
    ...
    # --- same-chars ---
    p_sc = cli.add_subcommand(
        "same-chars",
        "Check if strings have same characters",
        handler=run_same_chars,
        aliases=["sc"],
    )
    ...
    p_pat.add_argument(
        ...
        type=int,
        default=3,
        help="Minimum subsequence length (default: 3)",
    )
    _add_matcher_args(p_pat)

    # --- algorithm ---
    p_algo = cli.add_subcommand(
        "algorithm",
        "Show algorithm selection info",
        handler=run_algorithm_info,
        aliases=["algo"],
    )
