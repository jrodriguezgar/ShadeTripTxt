"""
Configuration Module for TextAnonymizer
...
Usage:
    from shadetriptxt.text_anonymizer.config import Config, load_config

    # Quick setup
    config = load_config(filepath='anonymizer_config.json')

    # Full control with method chaining
    config = Config(defaults={'locale': 'es_ES', 'strategy': 'redact'})
    config.load_file('config.json').load_env(prefix='ANONYMIZER_')
    ...
    schema.add_field('locale', type=str, required=True,
                     choices=['es_ES', 'en_US', 'en_GB', 'pt_BR', 'pt_PT',
                              ...
                              'es_CO', 'es_CL'])
    schema.add_field('min_confidence', type=float, min_value=0.0, max_value=1.0)
...
STANDARD_CONFIG_NAMES = [
    ...
    "anonymizer_config.yaml",
    "anonymizer_config.yml",
    "anonymizer_config.toml",
    "config.json",
    "config.yaml",
    "config.yml",
    "config.toml",
...
class Config:
    """Unified configuration manager for TextAnonymizer.
    ...
    def load_env(self, prefix: str = ENV_PREFIX) -> Config:
        ...
        Mapping: ANONYMIZER_KEY -> key, ANONYMIZER_NESTED__KEY -> nested.key

        Args:
            prefix: Environment variable prefix (default: 'ANONYMIZER_').
        """
    ...
    def get(self, key: str, default: Any = None, type: Optional[Type[T]] = None) -> Any:
        ...
        Args:
            key: Configuration key (use dot notation for nested: 'detection.use_regex').
...
def load_config(
    ...
    Args:
        ...
        defaults: Default values dictionary.
        env_prefix: Environment variable prefix (default: 'ANONYMIZER_').

    Example:
        config = load_config(filepath='anonymizer_config.json', env_prefix='ANONYMIZER_')
    """
    if defaults is None:
        defaults = {
            "locale": "es_ES",
...
def create_sample_config(filepath: str = "anonymizer_config.json") -> bool:
    """Create a sample TextAnonymizer configuration file.

    Args:
        filepath: Output file path (default: 'anonymizer_config.json').
    """
    sample = {
        "anonymizer": {
            "locale": "es_ES",
