"""
Configuration Module for TextParser CLI

Multi-source configuration with priority resolution.

Priority Order (highest to lowest):
    1. Command-line arguments
    2. Environment variables (TEXTPARSER_*)
    3. Configuration files (JSON, YAML, TOML)
    4. Default values

Usage:
    from shadetriptxt.text_parser.config import Config, load_config
    ...
    config.load_file('config.json').load_env(prefix='TEXTPARSER_')

    # Access values
    locale = config.get('locale')
    strength = config.get('strength', type=int)
...
STANDARD_CONFIG_NAMES = [
    "textparser_config.json",
    "textparser_config.yaml",
    "textparser_config.yml",
    "textparser_config.toml",
    "config.json",
    "config.yaml",
    "config.yml",
    "config.toml",
...
class Config:
    """Unified configuration manager for TextParser CLI.
    ...
    def load_env(self, prefix: str = ENV_PREFIX) -> Config:
        ...
        Mapping: TEXTPARSER_KEY -> key, TEXTPARSER_NESTED__KEY -> nested.key

        Args:
            prefix: Environment variable prefix (default: 'TEXTPARSER_').
        """
    ...
    def get(self, key: str, default: Any = None, type: Optional[Type[T]] = None) -> Any:
        ...
        Args:
            key: Configuration key (use dot notation for nested: 'parser.locale').
...
def load_config(
    ...
    Args:
        ...
        defaults: Default values dictionary.
        env_prefix: Environment variable prefix (default: 'TEXTPARSER_').

    Example:
        config = load_config(filepath='config.json', env_prefix='TEXTPARSER_')
    """
    if defaults is None:
        defaults = {
            "locale": "es_ES",
...
def create_sample_config(filepath: str = "textparser_config.json") -> bool:
    """Create a sample TextParser configuration file.

    Args:
        filepath: Output file path (default: 'textparser_config.json').
    """
    sample = {
        "parser": {
            "locale": "es_ES",
